# Idea for plot_countries: https://github.com/twiecki/covid19
...
def plot_countries(covid_df,
    ...
    for country in countries:
        ...
        if len(df_country) == 0:
            continue
        if norm_min_confirmed:
            df_country_0 = df_country[df_country.index[0]]
            df_country = df_country * (min_confirmed / df_country_0)
        if same_start:
            df_country.index = np.arange(len(df_country))
        if not ax:
            df_country.head(max_num_entries).plot(ls=ls,
                                                  marker=marker,
                                                  label=country + legendlabel)
        else:
            df_country.head(max_num_entries).plot(ls=ls,
                                                  marker=marker,
                                                  label=country + legendlabel,
