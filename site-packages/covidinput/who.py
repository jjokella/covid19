...
def coviddata_who(country, is_add_log=False, is_add_growth_factor=False):
    """
    Reading country input from WHO data
    """

    coviddata_who = coviddata_who_countries([country])
    coviddata_who.name = ("WHO-JHU " + country)

    if is_add_log:
        coviddata_who = cvdff.add_log(coviddata_who, country)

    if is_add_growth_factor:
        coviddata_who = cvdff.add_growth_factor(coviddata_who, country)
