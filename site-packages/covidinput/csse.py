...
def coviddata_csse(countries, switch=None):
    ...
    # Index to datetime
    coviddata_csse.index = pd.to_datetime(
        coviddata_csse.index)

    # Column names to countrynames
    coviddata_csse.columns = coviddata_csse_current[
        "Country/Region"]
    ...
    if "France" in countries:
        ...
        coviddata_csse = coviddata_csse.drop(
            "France", axis="columns")
    ...
    if "Netherlands" in countries:
        covid_france = coviddata_csse["Netherlands"].sum(
            axis="columns")
        coviddata_csse = coviddata_csse.drop(
