...
coviddata_rki = pd.read_csv(
    os.environ['HOME'] + "/covid19/covid-19-germany-gae/cases-rki-by-ags.csv",
    sep=',',
)

coviddata_risklayer = pd.read_csv(
    os.environ['HOME'] +
    "/covid19/covid-19-germany-gae/cases-rl-crowdsource-by-ags.csv",
    sep=',',
)
...
def coviddata_landkreise(switch="rki"):
    ...
    for rs in coviddata_landkreise.columns[1:]:
        ...
        if rs == "11000":
            ...
            landkreisname = list(landkreis_meta["GEN"][landkreis_meta["RS"] == np.int( rs)])[0]
