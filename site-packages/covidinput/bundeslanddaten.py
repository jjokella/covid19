...
coviddata_rki = pd.read_csv(
    os.environ['HOME'] + "/covid19/data/covid-19-germany-gae/cases-rki-by-state.csv",
    sep=',',
)

coviddata_risklayer = pd.read_csv(
    os.environ['HOME'] +
    "/covid19/data/covid-19-germany-gae/cases-rl-crowdsource-by-state.csv",
    sep=',',
)
...
def coviddata_bundeslaender(switch="rki"):
    """
    Dataframe with columns for Bundesländer and date=-index input from gae repository
    ...
    # Rename Länder from AGS specifier
    ...
    coviddata_bundeslaender.index = pd.to_datetime(
        coviddata_bundeslaender["time_iso8601"], utc=True)
