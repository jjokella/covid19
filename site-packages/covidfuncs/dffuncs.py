...
def add_mov_avg(covid_df, col, window=7.0):
    """
    Add moving average scaled to days
    """

    if type(covid_df.index) == pd.core.indexes.datetimes.DatetimeIndex:
        x = cvdf.get_x(covid_df)
    elif type(covid_df.index) == pd.core.indexes.numeric.Float64Index:
        x = pd.DataFrame(covid_df.index, index=covid_df.index, columns=["x"])
    else:
        raise TypeError("Index of covid_df has wrong type" +
                        type(covid_df.index))
    ...
    for index, row in covid_df.iterrows():
        ...
        # Mean in window
        ma = covid_df[col][(x["x"] <= x["x"][index])
                           & (x["x"] > x["x"][index] - window)].mean()

        covid_df.loc[index, ma_col] = ma


def add_mov_sum(covid_df, col, window=7.0):
    """
    Add moving sum scaled to days
    """

    x = cvdf.get_x(covid_df)

    ms_col = col + " SUM" + str(window)

    covid_df[ms_col] = covid_df[col]

    for index, row in covid_df.iterrows():

        # Sum in window
        ms = covid_df[col][x["x"] <= x["x"][index]][x["x"] > x["x"][index] -
                                                    window].mean()

        covid_df.loc[index, ms_col] = ms


def add_r(covid_df, col, gentime=7.0):
    """
    Add reproduction number r scaled to generation time in days.

    R in this case is computed as the sum of new cases
    in the previous gentime days divided by the sum of new cases
    in the gentime days before the previous gentime days.
    """

    x = cvdf.get_x(covid_df)

    r_col = col + " R" + str(gentime)

    if not col + " Diff" in covid_df.columns:
        add_diff(covid_df, col)

    sumcol = col + " Diff" + " SUM" + str(gentime - 0.1)
    if sumcol not in covid_df.columns:
        add_mov_sum(covid_df, col + " Diff", gentime - 0.1)

    covid_df[r_col] = covid_df[col]

    for index, row in covid_df.iterrows():

        if ((covid_df[sumcol][x["x"] <= x["x"][index] - gentime + 0.1][
                x["x"] > x["x"][index] - gentime - 0.1].size >= 1)
                and (covid_df[sumcol][x["x"] <= x["x"][index] - gentime + 0.1][
                    x["x"] > x["x"][index] - gentime - 0.1][0] > 0.0)):
            r = (covid_df[sumcol][index] /
                 (covid_df[sumcol][x["x"] <= x["x"][index] - gentime + 0.1][
                     x["x"] > x["x"][index] - gentime - 0.1][0]))
        else:
            r = np.NaN

        covid_df.loc[index, r_col] = r
...
def add_tdouble(covid_df, col, window=7.0, mincol=100):
    ...
    for index, row in covid_df.iterrows():
        ...
        # Size in window
        covidsize = covid_df[col][covid_df[col] < tmp_maxcol][
            covid_df[col] > tmp_mincol].size

        # Mean in window
        if covidsize >= window:
            td = get_tdouble(covid_df,
                             col,
                             mincol=tmp_mincol,
                             maxcol=tmp_maxcol)
        else:
...
def get_tdouble(covid_df, col, mincol=100, maxcol=20000):
    ...
    log_fit = get_log_fit(covid_df, col, mincol, maxcol)
...
def get_tdouble_dates(covid_df,
                      col,
                      startdat=pd.Timestamp("2020/03/01"),
                      enddat=pd.Timestamp("2020/12/31")):
    """
    Get the doubling time from dates.
    """

    log_fit = get_log_fit_dates(covid_df, col, startdat, enddat)
...
def get_r0(covid_df, col, gamma=1.0 / 16.6, mincol=100, maxcol=20000):
    ...
    tdouble = get_tdouble(covid_df, col, mincol=mincol, maxcol=maxcol)
