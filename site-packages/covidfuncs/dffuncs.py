...
def add_exponential_fit(covid_df, col, startdat):
    ...
    """
    x = cvdf.x_dates[covid_df.index[0]:covid_df.index[-1]]

    fit = np.poly1d(
        np.polyfit(x[startdat:]["x"], covid_df[col + " (Log)"][startdat:], 1))

    covid_df["Fit (Log)"] = pd.Series(fit(x["x"]), index=covid_df.index)
    covid_df["Fit (Exp)"] = pd.Series(
        np.exp(np.log(10) * fit(x["x"])),
        index=covid_df.index,
    )
    covid_df["Fit (Growth Factor)"] = covid_df["Fit (Exp)"] / covid_df[
        "Fit (Exp)"].shift(1)
...
def get_logistic_fit(covid_df, col, startdat):
    ...
    L, k, x0, y0 = sco.curve_fit(
        logistic,
        x[startdat:]["x"],
        covid_df["Confirmed"][startdat:],
        method="trf",
        p0=[8000.0, 0.5, 10.0, 0.0],
        sigma=[
            1.0 for i in range(len(covid_df["Confirmed"][startdat:].index))
        ],
        absolute_sigma=True,
        check_finite=True)[0]
...
def add_logistic_fit(covid_df, col, startdat):
    ...
    covid_df["Fit (Logistic)"] = logistic(x, L, k, x0, y0)
