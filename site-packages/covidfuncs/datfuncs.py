...
zero_dat = pd.Timestamp("2020/03/01T00:00:00+00:00")
...
x_dates = pd.DataFrame([
    dat_to_int(dat) for dat in [
        pd.Timestamp("2019/01/01T00:00:00+00:00") + pd.Timedelta(days=i)
        for i in range(731)
    ]
],
                       index=[
                           pd.Timestamp("2019/01/01T00:00:00+00:00") +
                           pd.Timedelta(days=i) for i in range(731)
                       ],
                       columns=["x"])

x_hours = pd.DataFrame([
    dat_to_int(dat) for dat in [
        pd.Timestamp("2019/01/01T00:00:00+00:00") + pd.Timedelta(hours=i)
        for i in range(731 * 24)
    ]
],
                       index=[
                           pd.Timestamp("2019/01/01T00:00:00+00:00") +
                           pd.Timedelta(hours=i) for i in range(731 * 24)
                       ],
                       columns=["x"])
...
def add_x_col(covid_df, col):
    """
    Add numerical array corresponding to Datetimes for fits
    """

    x = get_x_col(covid_df, col)

    covid_df["x" + col] = x["x"]


def get_x_col(covid_df, col):
    """
    Get numerical array corresponding to Datetimes for fits
    """
    ...
    x = pd.DataFrame([
        datdelta.delta / 10.0**9 / 3600.0 / 24.0
        for datdelta in covid_df[col] - zero_dat
    ],
                     index=covid_df.index,
                     columns=["x"])
